...
import { createMedicalProvider } from '../../utils/database';
...
export default function AddProviderModal({ isOpen, onClose, onSuccess }: Props) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await createMedicalProvider({ 
        ...
        phone_3_type: phones[2]?.type || null,
        phone_3: phones[2]?.number?.trim() || null,
        fax_1_type: faxes[0]?.type || null,
        fax_1: faxes[0]?.number?.trim() || null,
        fax_2_type: faxes[1]?.type || null,
        fax_2: faxes[1]?.number?.trim() || null,
        fax_3_type: faxes[2]?.type || null,
        fax_3: faxes[2]?.number?.trim() || null,
      ...
      setError('Could not save provider. Please try again.');
      setSaving(false);
    }
  };
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Add Medical Provider" maxWidth="lg">

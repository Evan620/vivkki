...
import { updateAutoInsurer, fetchAutoInsurers } from '../../utils/database';
import { Plus, Minus, ChevronDown, ChevronUp } from 'lucide-react';

interface Props {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: (updatedInsurer?: any) => void;
  insuranceId: number | null;
}

const CONTACT_TYPES = ['Main', 'Claims', 'Billing', 'Other'];

export default function EditAutoInsuranceModal({ isOpen, onClose, onSuccess, insuranceId }: Props) {
  // Company Information
  ...
  const disabled = !name.trim() || !insuranceId;

  // Load insurance data when modal opens
  useEffect(() => {
  ...
  const loadInsuranceData = async () => {
    ...
    try {
      ...
      const insurer = insurers.find(i => i.id === insuranceId);
      ...
      setStreetAddress(insurer.street_address || '');
      setStreetAddress2(insurer.street_address_2 || '');
      setCity(insurer.city || '');
      setState(insurer.state || 'OK');
      setZipCode(insurer.zip_code || '');
      setNotes(insurer.notes || '');
      
      // Populate phones
      const phoneArray: Array<{ type: string; number: string }> = [];
      if (insurer.phone_1) {
        phoneArray.push({ type: insurer.phone_1_type || 'Main', number: insurer.phone_1 });
      }
      if (insurer.phone_2) {
        phoneArray.push({ type: insurer.phone_2_type || 'Main', number: insurer.phone_2 });
      }
      // Fallback to legacy phone field
      if (phoneArray.length === 0 && insurer.phone) {
        phoneArray.push({ type: 'Main', number: insurer.phone });
      }
      if (phoneArray.length === 0) {
        phoneArray.push({ type: 'Main', number: '' });
      }
      setPhones(phoneArray);
      
      // Populate faxes
      const faxArray: Array<{ type: string; number: string }> = [];
      if (insurer.fax_1) {
        faxArray.push({ type: insurer.fax_1_type || 'Main', number: insurer.fax_1 });
      }
      if (insurer.fax_2) {
        faxArray.push({ type: insurer.fax_2_type || 'Main', number: insurer.fax_2 });
      }
      // Fallback to legacy fax field
      if (faxArray.length === 0 && insurer.fax) {
        faxArray.push({ type: 'Main', number: insurer.fax });
      }
      if (faxArray.length === 0) {
        faxArray.push({ type: 'Main', number: '' });
      }
      setFaxes(faxArray);
      
      // Populate emails
      const emailArray: Array<{ type: string; address: string }> = [];
      if (insurer.email_1) {
        emailArray.push({ type: insurer.email_1_type || 'Main', address: insurer.email_1 });
      }
      if (insurer.email_2) {
        emailArray.push({ type: insurer.email_2_type || 'Main', address: insurer.email_2 });
      }
      // Fallback to legacy email field
      if (emailArray.length === 0 && insurer.email) {
        emailArray.push({ type: 'Main', address: insurer.email });
      }
      if (emailArray.length === 0) {
        emailArray.push({ type: 'Main', address: '' });
      }
      setEmails(emailArray);
      
      // Auto-expand sections if they have data
      if (insurer.street_address || insurer.city || insurer.zip_code) {
        setShowAddressSection(true);
      }
      if (phoneArray.some(p => p.number) || faxArray.some(f => f.number) || emailArray.some(e => e.address)) {
  ...
  const handleSave = async () => {
    if (disabled || !insuranceId) return;
    setSaving(true);
    try {
      setError('');
      const updatedInsurer = await updateAutoInsurer(insuranceId, { 
        name: name.trim(), 
      ...
      console.error('Failed to update auto insurer', e);
      setError('Could not save auto insurance. Please try again.');
      setSaving(false);
    }
  };
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Edit Auto Insurance" maxWidth="lg">

...
import { supabase, fetchHealthInsurers, fetchHealthAdjusterById, updateHealthAdjuster, createHealthAdjuster } from '../utils/database';
import FormInput from '../components/forms/FormInput';
import FormSelect from '../components/forms/FormSelect';
import Toast from '../components/common/Toast';

export default function HealthAdjusterDetail() {
  ...
  const [healthInsurers, setHealthInsurers] = useState<any[]>([]);
  ...
  const loadHealthInsurers = async () => {
    try {
      const data = await fetchHealthInsurers();
      setHealthInsurers(data || []);
    } catch (error) {
      console.error('Error loading health insurers:', error);
    }
  };
  ...
  const fetchAdjuster = async () => {
    ...
    setIsLoading(true);
    try {
      const data = await fetchHealthAdjusterById(parseInt(id));
      if (data) {
        setAdjuster(data);
        setFormData({
          health_insurance_id: data.health_insurance_id?.toString() || '',
  ...
  const handleSave = async () => {
    ...
    if (!formData.health_insurance_id) {
      setToast({ message: 'Health insurance carrier is required', type: 'error' });
      return;
    }
    ...
    try {
      const adjusterData = {
        health_insurance_id: parseInt(formData.health_insurance_id),
      ...
      if (isNew) {
        const newAdjuster = await createHealthAdjuster(adjusterData);
        setToast({ message: 'Health adjuster created successfully', type: 'success' });
        navigate(`/health-adjusters/${newAdjuster.id}`);
      } else {
        await updateHealthAdjuster(parseInt(id!), adjusterData);
        setToast({ message: 'Health adjuster updated successfully', type: 'success' });
        setIsEditing(false);
        await fetchAdjuster();
      }
    } catch (error: any) {
      console.error('Error saving health adjuster:', error);
  ...
  const handleCancel = () => {
    if (isNew) {
      navigate('/health-adjusters');
    } else if (adjuster) {
      setFormData({
        health_insurance_id: adjuster.health_insurance_id?.toString() || '',
        ...
        fax: adjuster.fax || '',
        street_address: adjuster.street_address || '',
        city: adjuster.city || '',
      ...
      <DashboardLayout>
        <div className="flex items-center justify-center min-h-screen">
          <div className="text-center">
            ...
            <Button onClick={() => navigate('/health-adjusters')}>
  ...
  const adjusterName = isNew 
    ? 'New Adjuster' 
    : `${adjuster?.first_name || ''} ${adjuster?.middle_name ? `${adjuster.middle_name} ` : ''}${adjuster?.last_name || ''}`.trim();
    ...
    <DashboardLayout>
      <BreadcrumbNav
        items={[
          ...
          { label: 'Adjusters', href: '/health-adjusters' },
          { label: adjusterName }
        ]}
        className="mb-4"
      />
      ...
      <PageHeader
        title={adjusterName}
        description={isNew ? 'Create a new health insurance adjuster' : 'View and edit health insurance adjuster details'}
      >
        <div className="flex gap-2">
          {!isEditing ? (
            <>
              <Button variant="outline" size="sm" onClick={() => navigate('/health-adjusters')}>
                <ArrowLeft className="h-4 w-4 mr-2" />
                Back
              </Button>
            ...
            {isEditing ? (
              <div className="space-y-4">
                <FormSelect
                  ...
                  onChange={(value) => handleChange('health_insurance_id', value)}
                  options={[
                    { value: '', label: 'Select carrier...' },
                    ...healthInsurers.map(ins => ({ value: ins.id.toString(), label: ins.name }))
                  ]}
                  required
                />
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Health Insurance Carrier</label>
                  <p className="text-gray-900">{adjuster?.health_insurance?.name || 'N/A'}</p>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
